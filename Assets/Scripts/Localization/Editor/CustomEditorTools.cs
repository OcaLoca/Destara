    ...
    static public class CustomEditorTools
    {
        ...
        static public bool MakeTextureReadable(string path, bool force)
        {
            ...
            if (force || !settings.readable || settings.npotScale != TextureImporterNPOTScale.None || settings.alphaIsTransparency)
            {
                ...
                if (CustomSettings.trueColorAtlas)
                {
        ...
        static bool MakeTextureAnAtlas(string path, bool force, bool alphaTransparency)
        {
            ...
            ti.maxTextureSize < 4096 ||
                (CustomSettings.trueColorAtlas && ti.textureCompression != TextureImporterCompression.Uncompressed) ||
#else
        ...
        static public IntVector IntPair(string prefix, string leftCaption, string rightCaption, int x, int y)
        {
            ...
            CustomEditorTools.SetLabelWidth(48f);
            ...
            CustomEditorTools.SetLabelWidth(80f);
        ...
        static public Rect IntRect(string prefix, Rect rect)
        {
            ...
            CustomEditorTools.IntVector a = CustomEditorTools.IntPair(prefix, "Left", "Top", left, top);
            CustomEditorTools.IntVector b = CustomEditorTools.IntPair(null, "Width", "Height", width, height);
        ...
        static public Vector4 IntPadding(string prefix, Vector4 v)
        {
            ...
            CustomEditorTools.IntVector a = CustomEditorTools.IntPair(prefix, "Left", "Top", left, top);
            CustomEditorTools.IntVector b = CustomEditorTools.IntPair(null, "Right", "Bottom", right, bottom);
        ...
        static public void DrawTexture(Texture2D tex, Rect rect, Rect uv, Color color, Material mat)
        {
            ...
            // Draw the background
            CustomEditorTools.DrawTiledTexture(outerRect, CustomEditorTools.backdropTexture);
        ...
        static public bool DrawHeader(string text) { return DrawHeader(text, text, false, CustomSettings.minimalisticLook); }

        /// <summary>
        /// Draw a distinctly different looking header label
        /// </summary>

        static public bool DrawHeader(string text, string key) { return DrawHeader(text, key, false, CustomSettings.minimalisticLook); }
        ...
        static public void BeginContents() { BeginContents(CustomSettings.minimalisticLook); }
        ...
        static public SerializedProperty DrawProperty(string label, SerializedObject serializedObject, string property, bool padding, params GUILayoutOption[] options)
        {
            ...
            if (sp != null)
            {
                if (CustomSettings.minimalisticLook) padding = false;
        ...
        static public void DrawArray(this SerializedObject obj, string property, string title)
        {
            ...
            if (sp != null && CustomEditorTools.DrawHeader(title))
            {
        ...
        static public void DrawBorderProperty(string name, SerializedObject serializedObject, string field)
        {
            if (serializedObject.FindProperty(field) != null)
            {
                GUILayout.BeginHorizontal();
                {
                    ...
                    CustomEditorTools.SetLabelWidth(50f);
                    GUILayout.BeginVertical();
                    CustomEditorTools.DrawProperty("Left", serializedObject, field + ".x", GUILayout.MinWidth(80f));
                    CustomEditorTools.DrawProperty("Bottom", serializedObject, field + ".y", GUILayout.MinWidth(80f));
                    GUILayout.EndVertical();

                    GUILayout.BeginVertical();
                    CustomEditorTools.DrawProperty("Right", serializedObject, field + ".z", GUILayout.MinWidth(80f));
                    CustomEditorTools.DrawProperty("Top", serializedObject, field + ".w", GUILayout.MinWidth(80f));
                    GUILayout.EndVertical();
        ...
        static public void DrawRectProperty(string name, SerializedObject serializedObject, string field, float labelWidth, float spacing)
        {
            if (serializedObject.FindProperty(field) != null)
            {
                GUILayout.BeginHorizontal();
                {
                    ...
                    CustomEditorTools.SetLabelWidth(20f);
                    GUILayout.BeginVertical();
                    CustomEditorTools.DrawProperty("X", serializedObject, field + ".x", GUILayout.MinWidth(50f));
                    CustomEditorTools.DrawProperty("Y", serializedObject, field + ".y", GUILayout.MinWidth(50f));
                    GUILayout.EndVertical();

                    CustomEditorTools.SetLabelWidth(50f);
                    GUILayout.BeginVertical();
                    CustomEditorTools.DrawProperty("Width", serializedObject, field + ".width", GUILayout.MinWidth(80f));
                    CustomEditorTools.DrawProperty("Height", serializedObject, field + ".height", GUILayout.MinWidth(80f));
                    GUILayout.EndVertical();
        ...
        {
            if (!CustomSettings.minimalisticLook)
                GUILayout.Space(18f);
        }
        ...
        [MenuItem("Starwork/Localization/Generate localization file")]
        public static void GenerateLocalizationFile()
        {
            TextAsset asset = Resources.Load<TextAsset>("Localization");
            if (asset != null) { Debug.LogFormat("Localization file is already available at {0}", AssetDatabase.GetAssetPath(asset)); return; }

            asset = Resources.Load<TextAsset>("LocalizationExample");
            if (asset == null) { Debug.LogError("LocalizationExample file is missing from package resources, try updating the package..."); return; }
            string resourcesFolderPath = Path.Combine(Application.dataPath, "Resources/");
            try
            {
                string localizationFilePath = Path.Combine(resourcesFolderPath, "Localization.txt");
                if (!Directory.Exists(resourcesFolderPath))
                {
                    Directory.CreateDirectory(resourcesFolderPath);
                }
                File.WriteAllText(localizationFilePath, asset.text);
                AssetDatabase.SaveAssets();
                AssetDatabase.Refresh();
                Localization.LoadDictionary();
                bool localizationAvailableAndLoaded = !string.IsNullOrEmpty(Localization.Get("Language"));
                if (localizationAvailableAndLoaded)
                {
                    Debug.LogFormat("Localization file created at: {0}, current language is: {1}", localizationFilePath, Localization.language);
                }
            }
        ...
        [MenuItem("Starwork/Localization/Reload %&l")] //ctrl + alt + L
        static void ReloadLocalization()
        {
            Localization.LoadDictionary();
            Debug.Log("reloaded localization");
        }
    }
}